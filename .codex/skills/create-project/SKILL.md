---
title: create-project/SKILL.md
type: skill
name: create-project
description: 新建项目流程。建项目目录 + `_overview.md` + 同步 portfolio。AGENTS.md §项目工作加载判定为新建项目时调用。
---
...
**上游**：`AGENTS.md §项目工作加载` — 判定为新建项目时调用。
...
- **项目名称**（目录名 + `_overview.md` 标题）——优先英文（`[项目 1 占位]` / `[项目 4 占位]` / `[项目 5 占位]`）；纯业务中文场景可保留中文
- **项目目标**（一两句话）
