---
title: week-sync/SKILL.md
type: skill
name: week-sync
description: 专注区状态同步。每次会话启动步骤 6 自动调用（无需用户触发）。根据星期几分两档：周一至周四轻量同步，周五至周日深度回溯。两档均包含接续检查。
---

# week-sync

专注区状态同步权威源。由 `AGENTS.md §B 启动序列` 步骤 6 每次会话启动时自动调用。

## 加载链

**上游**：`AGENTS.md §B 启动序列` 步骤 6 — 每次会话自动调用。

**下游**：`<ASSISTANT_ROOT>\00 专注区\_本周.md`（深度回溯时写入任务标记）

---

## 周日特殊检查（任何一档均先执行）

如果当日为**周日**：

1. 检查 `_本周.md` 是否已有 `### 本周产出` 节
2. **有** → 说明本周复盘已完成，正常执行下方同步流程
3. **无** → 在同步报告末尾提示：「今天是周日，本周复盘还没做。要现在开始吗？」（等用户回应，不自动触发）

---

## 两档共同包含：接续检查（每次必须执行）

在同步报告的末尾，执行接续检查：

1. 读取 `_本周.md` 进展记录，找到**最近一次有实质工作的日期**（跳过空白日）
2. 从该日期节中提取所有工作段的：
   - 项目名称
   - 「关联：」字段中的文件列表
3. 在报告末尾附上接续提示：

```
---
**上次工作**（YYYY-MM-DD）：
- [项目名]：[当天做了什么，一句话]
  → 关联文件：[文件名1] · [文件名2]

要继续的话，告诉我，我先读一下相关文件再给你汇总。
```

> 如果用户当天第一句话就是「继续」「继续昨天」「从哪里开始」等，直接读取关联文件，给出完整汇总，不等确认。

---

## 轻量同步（周一至周四）

1. 读取 `_本周.md`，确认当前周次、任务清单和进展记录
2. 向用户简报：本周第几天、任务完成状态（X/N 完成）、近期进展一句话摘要
3. 执行接续检查（见上）
4. 不修改文件，不询问细节

---

## 深度回溯（周五至周日）

核心逻辑：**从 `_本周.md` 进展记录识别活跃项目，扫描活跃项目文件夹发现漏记文件，再核对任务状态。**

1. **读取 `_本周.md` 全文**，提取：
   - 任务清单：所有任务项及完成状态
   - 进展记录：从每天工作段「关联：」字段识别活跃项目路径 + 已记录文件名列表
   - 本周开始日期（从 frontmatter `dates` 字段）

2. **识别活跃项目**：从关联字段中提取 `01 项目区/` 下的项目名，得到活跃项目路径列表

3. **差集扫描（发现漏记文件）**：
   - 扫描每个活跃项目文件夹，收集**修改时间 ≥ 本周开始日期**的 `.md` 文件
   - 排除：`_archive/`、`_overview.md`、`文献记录.md` 等非工作产出文件
   - **差集** = 本周修改的文件 - 关联字段已记录的文件名
   - 对差集里每个文件，读前 30 行，理解内容和用途

4. **加载活跃项目概览**：对每个活跃项目，读取 `_overview.md`，了解当前状态

5. **核对任务完成状态**：
   - 对照任务清单，识别「进展记录里有推进但任务清单未标完成」的条目

6. **更新 `_本周.md`**（静默写入）：
   - 将有进展记录支撑的待办项标记为完成（明确完成的才标）
   - 不确定的标注「待确认」

7. **向用户报告**：
   - 本周活跃了哪些项目，各自推进到什么位置
   - **差集文件**：发现了哪些漏记文件、推断内容，建议补充到进展记录（不自动写入，日期归属由你确认）
   - 更新了哪些任务状态
   - 待确认项
   - 执行接续检查（见上）

> **核心原则：以活跃项目文件夹的实际修改为基准，发现漏记文件；以关联字段为索引，定向加载活跃项目，不扫全库。**
