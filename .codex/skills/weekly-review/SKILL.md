...
| skill | `~/.codex/skills\` 对应 skill | 触发条件 + 执行动作 + 边界 |
...
weekly-review 完成所有文件写入后，运行：

```text
<PYTHON_BIN> "<CODEX_HOME>/hooks/bom_check.py" "<ASSISTANT_ROOT>"
```

本检查只清理 Codex / Windows 写文件时可能产生的 UTF-8 BOM；不注入对话、不改变语义内容、不用于 Claude 端。
